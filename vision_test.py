...
class TestMode(Enum):
    E_CHART = "e_chart"           # 传统E字表
    LANDOLT_C = "landolt_c"       # 兰道尔C环
    SNELLEN = "snellen"           # 斯内伦字母表
    COLOR_VISION = "color_vision"  # 色觉测试
    CONTRAST = "contrast"         # 对比度测试
    PERIPHERAL = "peripheral"     # 周边视野测试

class VisionTester:
    def __init__(self, test_mode=TestMode.E_CHART):
        # 初始化摄像头和MediaPipe组件
        ...
        # 加载测试图片
        self.current_image = self._load_current_test_image()
        self._init_test_parameters()
        
        # 新增：自适应算法参数
        self.adaptive_threshold = 0.7  # 自适应阈值
        self.confidence_history = []   # 置信度历史
        self.difficulty_level = 1.0   # 难度等级
        
    def _load_test_images(self):
        """加载不同测试模式的图片"""
        images = {}
        test_images_dir = "pictures/test_images"
        
        # 确保测试图片目录存在
        if not os.path.exists(test_images_dir):
            os.makedirs(test_images_dir, exist_ok=True)
            self._create_default_test_images(test_images_dir)
        
        # 加载不同模式的测试图片
        for mode in TestMode:
            image_path = os.path.join(test_images_dir, f"{mode.value}.png")
            if os.path.exists(image_path):
                images[mode] = cv2.imread(image_path, cv2.IMREAD_UNCHANGED)
            else:
                # 使用默认E字图片作为备选
                images[mode] = cv2.imread('pictures/E.png', cv2.IMREAD_UNCHANGED)
        
        return images
    
    def _create_default_test_images(self, test_images_dir):
        """创建默认测试图片"""
        # 创建兰道尔C环
        c_ring = self._create_landolt_c()
        cv2.imwrite(os.path.join(test_images_dir, "landolt_c.png"), c_ring)
        
        # 创建斯内伦字母
        snellen = self._create_snellen_letters()
        cv2.imwrite(os.path.join(test_images_dir, "snellen.png"), snellen)
        
        # 创建色觉测试图
        color_test = self._create_color_vision_test()
        cv2.imwrite(os.path.join(test_images_dir, "color_vision.png"), color_test)
    
    def _create_landolt_c(self):
        """创建兰道尔C环"""
        img = np.ones((200, 200, 4), dtype=np.uint8) * 255
        cv2.circle(img, (100, 100), 80, (0, 0, 0, 255), 20)
        # 在圆环上开一个缺口
        cv2.rectangle(img, (100, 20), (120, 40), (255, 255, 255, 255), -1)
        return img
    
    def _create_snellen_letters(self):
        """创建斯内伦字母"""
        img = np.ones((200, 200, 4), dtype=np.uint8) * 255
        cv2.putText(img, "E", (50, 120), cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 0, 0, 255), 5)
        return img
    
    def _create_color_vision_test(self):
        """创建色觉测试图"""
        img = np.ones((200, 200, 4), dtype=np.uint8) * 255
        # 创建彩色圆点图案
        colors = [(255, 0, 0), (0, 255, 0), (0, 0, 255)]  # 红绿蓝
        for i, color in enumerate(colors):
            cv2.circle(img, (50 + i*50, 100), 15, (*color, 255), -1)
        return img
    
    def _load_current_test_image(self):
        """加载当前测试模式的图片"""
        return self.test_images.get(self.test_mode, self.test_images[TestMode.E_CHART])
    ...
    def switch_test_mode(self, new_mode):
        """切换测试模式"""
        if isinstance(new_mode, str):
            new_mode = TestMode(new_mode)
        
        self.test_mode = new_mode
        self.current_image = self._load_current_test_image()
        self._init_test_parameters()
        print(f"已切换到{new_mode.value}测试模式")
    ...
    def adaptive_difficulty(self):
        ...
        recent_accuracy = sum(self.confidence_history[-3:]) / 3
        
        if recent_accuracy > 0.8:  # 准确率很高，增加难度
            self.difficulty_level = min(2.0, self.difficulty_level * 1.1)
            self.current_size = max(self.min_size, self.current_size * 0.9)
        elif recent_accuracy < 0.6:  # 准确率较低，降低难度
            self.difficulty_level = max(0.5, self.difficulty_level * 0.9)
            self.current_size = min(self.max_size, self.current_size * 1.1)

    def get_comprehensive_vision_report(self):
        """获取综合视力报告"""
        report = {
            'test_mode': self.test_mode.value,
            'final_vision': self._calculate_final_vision(),
            'accuracy_rate': self.correct_count / max(self.total_count, 1),
            'average_response_time': np.mean(self.test_results['timing']) if self.test_results['timing'] else 0,
            'difficulty_progression': self.difficulty_level,
    ...
    def _generate_recommendations(self):
        ...
        if final_vision < 4.0:
            recommendations.append("建议尽快就医进行专业视力检查")
        elif final_vision < 4.8:
            recommendations.append("建议定期进行视力检查")
        
        if accuracy < 0.7:
            recommendations.append("测试过程中注意力可能不够集中，建议重新测试")
        
        if self.test_mode == TestMode.COLOR_VISION:
            recommendations.append("色觉测试结果正常")
    ...
    def _update_display(self, frame):
        """更新显示内容"""
        # 生成旋转后的E字
        rotated_e = self.rotate_image(self.current_image, self.current_direction)
